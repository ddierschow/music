...
\book
{
  ...
  \score
  {
	...
	<<
	  ...
	  {
	    #(set-accidental-style 'modern)
	    \set Staff.instrumentName = \markup { \column { "Horns I,II" \line { "in G" } } }
	    %\set Staff.shortInstrumentName = "Hn.1/2"
	    \set Staff.shortInstrumentName = \markup { \column { "Hn." \line { "1/2" } } }

	      \new Voice {\keepWithTag #'score \relative c <<
		\outlineMvtIII
		\partcombine \hornGOneMvtIIIScore \hornGTwoMvtIIIScore
	    >> }
	  }
	  \new Staff
	  {
	    #(set-accidental-style 'modern)
	    \set Staff.instrumentName = \markup { \column { "Horns III,IV" \line { "in C" } } }
	    %\set Staff.shortInstrumentName = "Hn.3/4"
	    \set Staff.shortInstrumentName = \markup { \column { "Hn." \line { "3/4" } } }
	      \new Voice {\keepWithTag #'score \relative c <<
		\partcombine \hornThreeMvtIIIScore \hornFourMvtIIIScore
	    >> }
	  }
	>>
	\new Staff
	{
	  ...
	  \set Staff.shortInstrumentName = "Tr."
	    \new Voice {\keepWithTag #'score \relative c <<
		\partcombine \trumpetOneMvtIIIScore \trumpetTwoMvtIIIScore
	  >> }
	}
	\new Staff
	{
